...
class Cn extends Component {
  ...
  initTopDomain = async () => {
    const TopData = await GetTopDomainsByBandwidth({ Limit: 20 });
  ...
  checkPage = async () => {
    try {
      ...
      const { BandwidthStatistics } = await GetBandwidthStatistics({
        ...this.param
      });
      ...
      <div className="common-bottom">
        <div>
          <Spin tip="查询数据中..." spinning={this.state.spinning}>
            <Search onSubmit={this.search} exportFunc={'GetBandwidthStatistics'}/>

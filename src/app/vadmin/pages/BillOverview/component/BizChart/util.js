...
export function scale(scaleType) {
  return {
    Timestamp: {
      alias: '时间',
      type: scaleType,
...
export const genFormatTooltip = type => (Timestamp, Value, Key,Detail) => {
  return {
    Value: Number.isNaN(Value) || Value == null ? '暂无数据' : format(type, Value),
    Key,
    value: Value,
    Detail
  };
};
export function htmlContent(title, items, sort = false) {
